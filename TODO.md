...
### 4. `sql/metrics_1s.sql` assumes the table is in `public`

`sql/metrics_1s.sql:179` `GRANT USAGE ON SCHEMA public` and a hard-coded
...
`README.md:265` says `psql "$PG" -f sql/metrics_1s.sql`, but the Configure
...
`README.md:265` uses `$PG`; `sql/metrics_1s.sql:6` uses `$DSN`.
